...
contract C1 {
    ...
    function callSetNum(address c2, uint _num) public {
        if(!c2.call(bytes4(keccak256("setNum(uint256)")), _num)) revert(); // C2's num is set 
    }
    ...
    function callcodeSetNum(address c2, uint _num) public {
        if(!c2.callcode(bytes4(keccak256("setNum(uint256)")), _num)) revert(); // C1's num is set
    }

    function delegatecallSetNum(C2 c2, uint _num) public {
        address s2 = address(c2);
        if(!s2.delegatecall(bytes4(keccak256("setNum(uint256)")), _num)) revert(); // C1's num is set 
    }
}
